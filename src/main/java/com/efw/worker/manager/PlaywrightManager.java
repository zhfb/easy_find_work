package com.efw.worker.manager;

import com.fasterxml.jackson.databind.ObjectMapper;
import com.efw.application.entity.CookieEntity;
import com.efw.application.service.CookieService;
    ...
    private void setup51jobPlatform() {
        ...
        try {
            ...
            if (!checkIf51jobLoggedIn()) {
                log.info("51job：未登录，请在浏览器中手动登录（不会自动跳转）");
            } else {
                log.info("51job已登录");
            }
        } catch (Exception e) {
    ...
    private boolean checkIfZhilianLoggedIn() {
        try {
            ...
            if (loginButtonExists) {
                log.info("智联招聘：未登录，请在浏览器中手动登录（不会自动跳转）");
                return false;
            }

            // 尝试通过其他元素判断是否已登录
            try {
                if (zhilianPage.locator("a.home-header__c-login, .user-info, [class*='user']").count() > 0) {
                    isLoggedIn = true;
                }
            } catch (Exception ignored) {}

            if (isLoggedIn) {
                log.debug("智联招聘登录检测：存在用户信息元素，判定已登录");
            } else {
                log.debug("智联招聘登录检测：未匹配到明确特征，判定未登录");
            }

            return isLoggedIn;
        } catch (Exception e) {
            log.debug("智联招聘登录检测异常: {}", e.getMessage());
            return false;
        }
    }
