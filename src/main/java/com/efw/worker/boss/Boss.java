package com.efw.worker.boss;

import com.efw.application.entity.AiEntity;
import com.efw.application.service.AiService;
import com.efw.application.service.BossService;
...
import com.efw.worker.utils.Job;
import com.efw.worker.utils.JobUtils;
import com.efw.worker.utils.PlaywrightUtil;
...
import static com.efw.worker.boss.Locators.*;
...
public class Boss {
        ...
        for (String keyword : config.getKeywords()) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (jobDesc != null && !jobDesc.isEmpty()) {
                    try {
                        String evalCompany = bossCompany != null ? bossCompany : "未知公司";
                        String evalJobName = jobName != null ? jobName : "未知岗位";
                        String evalSalary = jobSalary != null ? jobSalary : "";

                        JobInfo jobInfo = new JobInfo("", evalCompany, evalJobName, evalSalary, jobDesc);
                        if (bossActive != null) jobInfo.setHrActiveStatus(bossActive);

                        EvaluationResult evalResult = evaluationService.evaluate(jobInfo, null);
                        double threshold = config.getMinScoreThreshold() != null
                            ? config.getMinScoreThreshold() : EvaluationService.DEFAULT_THRESHOLD;

                        if (evalResult.getTotalScore() < threshold) {
                            log.info("评估未通过: {} | {} | 评分={} | 阈值={} | 原型={}",
                                evalCompany, evalJobName, evalResult.getTotalScore(), threshold, evalResult.getArchetype());
                            progressCallback.accept("跳过（评分不足）：" + evalJobName + " [" + evalResult.getTotalScore() + "/" + threshold + "]", i + 1, count);
                            continue;
                        }
                        log.debug("评估通过: {} | {} | 评分={}/{}", evalCompany, evalJobName, evalResult.getTotalScore(), threshold);
                    } catch (Exception e) {
                        log.warn("评估异常，放行投递: {}", e.getMessage());
                    }
                }
    ...
    private void processJobDetailJsonAndInsert(String body) {
        ...
        try {
            ...
            com.efw.application.entity.BossJobDataEntity entity = new com.efw.application.entity.BossJobDataEntity();
