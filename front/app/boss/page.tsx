      ...
      <Tabs defaultValue="config" className="w-full">
        ...
        <Card className="animate-in fade-in slide-in-from-bottom-7 duration-700">
          <CardHeader>
            ...
            <CardDescription>设置岗位自动评估门槛，低于阈值的岗位自动跳过</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <div className="space-y-2">
                <Label>最低评分阈值: {config.minScoreThreshold ?? 3.5}</Label>
                <input
                  type="range"
                  min="1"
                  max="5"
                  step="0.5"
                  value={config.minScoreThreshold ?? 3.5}
                ...
                <p className="text-xs text-muted-foreground mt-2">
                  评估维度：角色匹配(A) | 能力匹配(B) | 薪酬合理(C) | 公司文化(D) | 成长空间(E) | 面试准备(F) | 岗位合法性(G)
                </p>
              </div>
            </div>
          </CardContent>
