...
import { BiEnvelope, BiBriefcase, BiSearch, BiTask, BiUserCircle, BiBrain, BiMoon, BiSun, BiBarChart, BiLineChart, BiFile, BiMessage } from 'react-icons/bi'
...
export default function Sidebar() {
  ...
  const toolGroup = [
    { href: '/pipeline', icon: BiBarChart, label: '求职管道', color: 'text-green-300' },
    { href: '/resumes', icon: BiFile, label: '简历管理', color: 'text-orange-300' },
    { href: '/interview', icon: BiMessage, label: '面试准备', color: 'text-pink-300' },
  ]
    ...
    >
      ...
      >
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">EFW</h1>
      ...
      <nav className="p-4 space-y-4 overflow-y-auto h-[calc(100vh-280px)]">
        ...
        {/* 求职工具分组 */}
        <div>
          <div className="px-4 py-2 text-white dark:text-waterloo text-xs uppercase tracking-wide">求职工具</div>
          <div className="space-y-2">
            {toolGroup.map((item, index) => {
              const Icon = item.icon
              const isActive = pathname === item.href
              return (
                <motion.div
